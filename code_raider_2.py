...
def intro():
    ...
    print("You have to pass three levels collecting some items (please note that maximum weight is 12 kilos after \
reaching this limit you cannot acquire any new items!), do some \
...
def are_you_ready(key,total_time, time0, time1, tab, inventory_numbers, inventory_weight, cloth_armour_class):
    """Asks user if he is ready to play"""
    if key == "y":
        time_challenge(tab, total_time, time0, time1, inventory_numbers, inventory_weight, cloth_armour_class)
    elif key == "n":
        os.system("clear")
        f = open('idiot.txt', 'r')
        file_contents = f.read()
...
def time_challenge(tab, total_time, time0, time1, inventory_numbers, inventory_weight, cloth_armour_class):
    '''In time challenge player have to collect items in set time'''

    os.system("clear")
    print("Challenge 1\n\n")
    print("You have to collect 5 loot as fast as possible. Good luck!\n")
...
def boss_fight(inventory_weight, inventory_numbers, cloth_armour_class):
    ...
    while True:
        key = getch()
        user_input = input("\nYou have now #%d life points. Do you dare to insert the number? \n" % (char_hp))
        #if user_input == "h":
            #print_inventory(inventory_weight, inventory_numbers)
...
def main(tab, inventory_numbers, inventory_weight, cloth_armour_class):
    ...
    time1 = 0
    total_time = 0
    are_you_ready(key, total_time, time0, time1, tab, inventory_numbers, inventory_weight, cloth_armour_class)
